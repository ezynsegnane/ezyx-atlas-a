%==============================================================
%  EZNX-ATLAS-A -- English manuscript source
%  2026-05-12
%==============================================================
\documentclass[mathematics,article,pdftex]{Definitions/mdpi}
...
\makeatletter
\gdef\@journalshort{Manuscript}
\gdef\@journalfull{Manuscript}
\makeatother
...
\AtBeginDocument{%
  ...
  \hypersetup{
    ...
    pdfsubject={EZNX-ATLAS-A manuscript and reproducibility package},
    pdfkeywords={electrocardiogram, deep learning, multimodal fusion, missing data, PTB-XL}
  }%
}

% ---- Journal-template administrative metadata --------------
\firstpage{1}
...
\subsection{Methodological contribution and statistical framing}
...
\subsection{Pre-specified supplementary analyses}
The ancillary analyses listed in the pre-declared analysis plan (SHA-256 prefix/suffix: \texttt{4ad22e90}/\texttt{aea5c3dc}) are reported as aggregate supplementary repository artifacts generated by \path{scripts/complete_item8_supplementary_analyses.py}: metadata-only logistic-regression and XGBoost baselines, post-hoc metadata decomposition controls (\texttt{normal}, \texttt{shuffle\_val}, \texttt{mask\_only}, and \texttt{none}) over all 60 Group~A checkpoints without retraining, per-class AUPRC, Brier score, 15-bin ECE, and subgroup AUC by age tertile, metadata completeness, and sex. These analyses remain descriptive and do not change the confirmatory family, which is restricted to the three paired Group~A macro-AUC contrasts. The supplementary artifacts show metadata-only macro-AUCs of 0.6899 (logistic regression) and 0.6878 (XGBoost), a \texttt{demo+anthro} macro-AUPRC of 0.8218, macro-Brier score of 0.0961, and macro-ECE of 0.0754; for \texttt{demo+anthro}, post-hoc decomposition gives a mean normal-minus-none signal of 0.0014 macro-AUC and a mean normal-minus-\texttt{shuffle\_val} value-content signal of 0.0019. Patient-level probability arrays are not included in the public repository, but the aggregate CSV/JSON/Markdown outputs and reproduction script are included.

\subsection{Limitations}\label{sec:limits}

\begin{enumerate}[leftmargin=1.4em]
    \item \textbf{External validation and split scope.} The confirmatory Group~A results are conditioned on PTB-XL fold~10 and have not yet been tested on an external or prospective cohort. This limits generalisation across institutions, acquisition devices, demographic distributions, and temporal shifts. Group~F provides a descriptive multi-split absolute-performance check, but its alternative test folds overlap with the primary metadata-normalisation index and only the \texttt{demo+anthro} variant was re-evaluated; therefore Group~F supports context for fold-level variation rather than an independent validation claim.
    \item \textbf{Clinical utility.} The study reports discrimination and descriptive supplementary calibration summaries (AUC, AUPRC, Brier score, and 15-bin ECE), but it does not fit a recalibration model or evaluate decision-curve benefit, net reclassification, workflow impact, or prospective patient outcomes. The measured metadata gains should therefore be interpreted as reproducible benchmark increments, not as evidence of deployable clinical utility.
    \item \textbf{Task and metadata scope.} The analysis is restricted to the PTB-XL 5-superclass task and to age, sex, height, weight, and BMI. Additional clinical variables such as blood pressure, electrolytes, medication use, and echocardiographic measurements are absent. Age, sex, and body size may also carry population-structure signal, so the measured gains cannot be attributed to a single physiological mechanism.
    \item \textbf{Reproducibility environment.} All 250 runs were executed on a CPU-only Windows environment (Intel Core i5, 8\,GB RAM, PyTorch 2.3.1+cpu). CPU-only execution avoids CUDA-specific nondeterminism, but bitwise reproduction may still depend on CPU architecture, BLAS versions, threading behaviour, and the exact software stack. The repository therefore provides the source snapshot, environment files, and aggregate artifacts needed to reproduce the reported analyses under a matched environment.
\end{enumerate}
...
% -------------------------------------------------------------
\vspace{6pt}
\supplementary{Supplementary materials available in the public GitHub repository (\url{https://github.com/ezynsegnane/ezyx-atlas-a}) include: \textbf{Table~S1} --- the 3-test BH-FDR pre-specified family (Group~A pairwise macro-AUC contrasts) with raw and adjusted $p$-values; seed-level JSON outputs for all 250 runs across six groups; seed-level CSV/Markdown statistical tables; item~8 supplementary aggregate analyses (classical baselines, post-hoc metadata controls, per-class AUPRC, Brier/ECE, and subgroup AUC); dataset-integrity and statistical-protocol notes; environment files; and source scripts.}
...
\dataavailability{The PTB-XL v1.0.3 dataset is freely available on PhysioNet (\url{https://physionet.org/content/ptb-xl/1.0.3/}, doi:10.13026/kfzx-aw45; accessed 19 May 2026)~\cite{Wagner2020,Goldberger2000,PTBXLv103}. The complete training code, all 250 seed-level result JSON files across six experimental groups, derived Group~A statistical artifacts regenerated from all 60 seed JSONs (20 seeds $\times$ 3 variants), item~8 supplementary aggregate artifacts generated by \path{scripts/complete_item8_supplementary_analyses.py}, and the six manuscript figures are publicly available at \url{https://github.com/ezynsegnane/ezyx-atlas-a} (MIT licence; a versioned tag or Zenodo DOI will be created upon acceptance to provide a fixed reference). The pre-declared analysis plan (\texttt{analysis\_plan.md}) is included in the public repository; its SHA-256 is \texttt{4ad22e90\ldots{}aea5c3dc} (full hash in each run JSON under key \texttt{analysis\_plan\_sha256}), matching the plan used for the archived training runs. The public missingness report covers the \texttt{demo+anthro} variant across 20 seeds (2024--2043); \texttt{none} and \texttt{demo} masking curves are structural reference baselines immune to anthropometric masking by construction and are shown in Fig.~\ref{fig:missing} at their 20-seed means from Group~A. Patient-level probability arrays (per-patient class probabilities, labels, and split identifiers) were not individually archived due to storage constraints; the seed-level JSON files and item~8 aggregate outputs provide the reported AUC, AUPRC, Brier/ECE, post-hoc metadata-control, and subgroup summaries.}
...
\caption{Missingness robustness of the availability-aware fusion pathway. The \texttt{demo+anthro} empirical masking curve is evaluated across \textbf{20 seeds} (seeds 2024--2043); \texttt{none} and \texttt{demo} traces are structural reference baselines (immune to anthropometric masking by construction) shown at their 20-seed mean. Test macro-AUC is shown as a function of the fraction $\rho$ of anthropometric fields withheld at inference (MCAR, independently per field); dashed horizontal lines show the \texttt{demo} and \texttt{none} baselines. Full masking ($\rho{=}1.0$) costs $\approx 0.0011$ macro-AUC for \texttt{demo+anthro}, converging to the \texttt{demo} baseline (difference $+0.00011$, within one-tenth of the cross-seed SD); the \texttt{none} variant is immune by construction. Availability-mask updating is performed consistently at each rate.}
\label{fig:missing}
\end{figure}
